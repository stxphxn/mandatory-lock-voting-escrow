...
min_unlock_timestamp: public(uint256)
...
@external
def set_min_unlock_timestamp(_timestamp: uint256):
    """
    @notice Sets the minimum unlock timestamp for all locks
    @param _timestamp New minimum unlock timestamp
    """
    assert msg.sender == self.admin, "Only admin can set minimum unlock timestamp"
    assert _timestamp >= block.timestamp, "Timestamp must be in the future"
    self.min_unlock_timestamp = _timestamp
...
def create_lock(_value: uint256, _unlock_time: uint256):
    ...
    if self.min_unlock_timestamp > 0:
        assert unlock_time >= self.min_unlock_timestamp, "Unlock time must be at least the minimum unlock timestamp"
