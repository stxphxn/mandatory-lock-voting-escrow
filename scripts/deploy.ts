...
async function main() {
  ...
  const votingEscrowImpl = await deployAndVerify(
    "MandatoryLockVotingEscrow",
    []
  );
  ...
  // deploying launchpad
  const launchpad = await deployAndVerify("Launchpad", [
  ...
  console.log(
    "The MandatoryLockVotingEscrow Implementation deployed at:",
    votingEscrowImpl.address
  );
  console.log(
    "The RewardDistributor Implementation deployed at:",
    rewardDistributorImpl.address
  );
  console.log(
    "The RewardFaucet Implementation deployed at:",
    rewardFaucetImpl.address
  );

  console.log("The Launchpad deployed at:", launchpad.address);

  const abi = ["constructor(address,address,address)"];
